#!/bin/bash
# 원클릭 배포: Lightsail 더미 데이터 + GitHub Pages
...
if [ ! -f "${LIGHTSAIL_KEY}" ]; then
    echo "❌ 에러: SSH 키를 찾을 수 없습니다: ${LIGHTSAIL_KEY}"
    echo "LightsailDefaultKey.pem 파일을 홈 디렉토리에 복사하세요."
    exit 1
fi

# 1. 스크립트 전송 + 실행 (한 번에)
echo "📤 Lightsail에 더미 데이터 배포 중..."
ssh -i "${LIGHTSAIL_KEY}" "${LIGHTSAIL_USER}@${LIGHTSAIL_IP}" << 'ENDSSH'
cd /home/ubuntu/satellite_vehicle_tracker/backend
source venv/bin/activate

# 더미 데이터 생성 스크립트가 없으면 생성
...
echo ""
echo "✅ 배포 완료! 차량 수 확인:"
curl -s http://localhost:8000/api/abandoned-vehicles | python3 -c "import sys, json; print(f'{len(json.load(sys.stdin))}대')"
ENDSSH

echo ""
echo "✅ Lightsail 배포 완료!"
echo ""
echo "🌐 Cloudflare Tunnel로 즉시 확인:"
echo "   https://standings-classification-easy-textbook.trycloudflare.com/api/abandoned-vehicles"
echo ""
echo "📱 GitHub Pages는 자동으로 최신 데이터를 표시합니다:"
