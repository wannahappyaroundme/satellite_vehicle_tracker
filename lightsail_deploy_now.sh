#!/bin/bash
# Lightsail 서버에 더미 데이터 즉시 배포

cat << 'INSTRUCTIONS'
============================================================
Lightsail 브라우저 SSH에서 다음 명령어를 실행하세요
============================================================

1. AWS Lightsail 콘솔 접속:
   https://lightsail.aws.amazon.com/

2. satellite-backend 인스턴스 클릭

3. "Connect using SSH" 버튼 클릭 (주황색)

4. 아래 전체 명령어를 복사해서 붙여넣고 Enter:

============================================================
INSTRUCTIONS

cat << 'ENDSSH'
# 더미 데이터 생성 및 배포 (한 번에 실행)
...
echo ""
echo "✅ 배포 완료! 차량 수 확인:"
curl -s http://localhost:8000/api/abandoned-vehicles 2>/dev/null | python3 -c "import sys, json; data=json.load(sys.stdin); print(f'{len(data)}대')" 2>/dev/null || echo "API 응답 확인 중..."

echo ""
echo "🌐 Cloudflare Tunnel로 확인:"
...
echo ""
echo "============================================================"
echo "위 명령어를 Lightsail 브라우저 SSH에 붙여넣으세요!"
echo "============================================================"
